...
class ViewController: UIViewController {
    ...
    var withoutSubFolderWatcher: DirectoryDeepWatcher!
    ...
    override func viewDidLoad() {
        ...
        self.watcher.onNewFiles = { newFiles in
            ...
            for newFile in newFiles {
                if let fileWatcher = FileWatcher.watch(url: newFile, isReadFile: true) {
                    self.fileWatchers.append(fileWatcher)
                }
            }
        }
...
//		}
        ...
        self.withoutSubFolderWatcher = DirectoryDeepWatcher.watch(url, isWatchSubFolder: false)
        
        self.withoutSubFolderWatcher.onFileChangeNotification = { fileChange in
            let log = "==== file change notification: \n\(fileChange.description)"
            print(log)
            self.updateLogs(log)
        }
    }
